...
export default function OfflineClasses (props) {
    ...
    const getOfflineClasses = async() => {
        const q1 = query(collection(db, "subject"),
            where("department", "==", profile.department),
            where("year", "==", profile.year));
        const querySnapshot1 = await getDocs(q1);
        const subjects = querySnapshot1.docs.map(doc => doc.id);

        const q2 = query(collection(db, "offlineClass"), where('subjectName', 'in', subjects));
        const querySnapshot2 = await getDocs(q2);
        setOfflineClasses(querySnapshot2.docs.map((doc) => (
            { ...doc.data(), id: doc.id }
        )));
    }
                        ...
                        return (
                            <Card border="dark" style={{backgroundColor: "transparent", boxShadow: "0px 0px 10px grey"}} className="m-4">
                                ...
                                <Card.Body>
                                    ...
                                    <Card.Text>
                                        Faculty: { offlineClass.faculty }<br />
                                        Date: { offlineClass.date }<br />
                                        Time: { offlineClass.time }
                                        <Button className="float-end" variant="outline-dark">Register</Button>
                                    </Card.Text>
                                </Card.Body>
                            </Card>
                        );
