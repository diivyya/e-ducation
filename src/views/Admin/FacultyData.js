...
export default function FacultyData() {
    const initialFormValues = {
        name: '', email: '', facultyId: '', designation: '', phone: '', password: 'faculty123',
        department: '', gender: '', vaccinationStatus: '', dob: '', subjectsCanTeach: []
    }
    ...
    const getSubject = async() => {
        const q = query(collection(db, "subject"), where("department", "==", formValues.department));
        const querySnapshot = await getDocs(q);
        setSubject(querySnapshot.docs.map((doc) => (
            { name: doc.data(), id: doc.id }
        )));
    }

    const setSubjectsCanTeach = (event) => {
        setFormValues(oldValues => ({...oldValues, ["subjectsCanTeach"]: event.map((e) => e.id) }))
    }
    ...
    return (
        <div>
            <Button variant="outline-danger" className="mt-4 mb-4" onClick={showOnClick}>
                {(showCreateForm || openEditForm) ? "Close Form" : "Create new faculty"}</Button>
